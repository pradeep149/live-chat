...
export default function LiveVideoStream() {
  ...
  const enableLoop = () => {
    ...
    loopStart.current = Math.max(v.duration - 4, 0);
    loopActive.current = true;
    log("Looping last 4s…");
    v.currentTime = loopStart.current;
    v.play().catch(() => {});
  };
  ...
  const shouldExitLoop = () => {
    const v = videoRef.current;
    return loopActive.current && v.duration - loopStart.current > 4.05;
  };
  ...
  const handleTimeUpdate = () => {
    ...
    if (
      ...
      enableLoop();
    if (loopActive.current && v.currentTime >= v.duration - 0.05) {
      v.currentTime = loopStart.current;
      v.play().catch(() => {});
    }
  };
  ...
  const openSSE = () => {
    esRef.current = new EventSource(STREAM);
    esRef.current.addEventListener("chunk", (e) => fetchChunk(e.data.trim()));
    esRef.current.addEventListener("end", handleStreamEnd);
    esRef.current.onerror = () => log("SSE error");
  };
